...
import org.jsoup.nodes.{Element, TextNode, Document}
...
class HtmlToMarkdown {
  ...
  def parseElement(doc: Document) = doc.select("*").asScala.map(element => {
  ...
  def parseCode(doc: Document) = doc.getElementsByTag("code").asScala.map(h => {
  ...
  def parseBlockQuotes(doc: Document) = doc.getElementsByTag("blockquote").asScala.map(h => {
    val content = NEW_LINE + BLOCK_QUOTE + CONTENT_NEW_LINE + filterSpace(h.text()) + NEW_LINE
    updateElement(h, content)
  })

  def parseDiv(doc: Document) = doc.getElementsByTag("div").asScala.map(h => {
    updateElement(h, NEW_LINE + filterSpace(h.text()) + NEW_LINE)
  })

  def parseTable(doc: Document) = doc.getElementsByTag("table").asScala.filter(u => u.getElementsByTag("thead") == null)
    .map(h => {
  ...
  def parseUL(doc: Document) = doc.getElementsByTag("ul").asScala.map(h => {
  ...
  def parseParagraph(doc: Document) = doc.getElementsByTag("p").asScala.map(h => {
  ...
  def parseImage(doc: Document) = doc.getElementsByTag("img").asScala.map(h => {
    val content = "![" + h.attr("alt") + "]" + "(" + h.attr("src") + ")"
    updateElement(h, content)
  })

  def parseHref(doc: Document) = doc.getElementsByTag("a").asScala.map(h => {
    val content = "[" + filterSpace(h.text()) + "](" + h.attr("href") + ")"
    updateElement(h, content)
  })

  def parseHR(doc: Document) = doc.getElementsByTag(HR).asScala.map(h => {
    updateElement(h, getHr)
  })

  def parseHeader(doc: Document) = HEADER.map(t =>
  ...
  def updateElement(el: Element, content: String) = el.replaceWith(new TextNode(content, ""))
